...
async function Header() {
  // Check if we have valid environment variables - using same logic as provider
  ...
  const hasValidConvexUrl = process.env.NEXT_PUBLIC_CONVEX_URL && 
    process.env.NEXT_PUBLIC_CONVEX_URL.startsWith('https://') && 
    !process.env.NEXT_PUBLIC_CONVEX_URL.includes('your_') && 
    !process.env.NEXT_PUBLIC_CONVEX_URL.includes('happy-animal') &&
    process.env.NEXT_PUBLIC_CONVEX_URL.includes('.convex.cloud');

  let user = null;
  let convexUser = null;

  // Only try to get user data if we have valid keys
  if (hasValidClerkKey && hasValidConvexUrl) {
    try {
      ...
      // Gracefully handle errors when authentication is not properly configured
      console.log("Authentication not configured:", error);
    }
  }
